...
class Levels {
public:
	Levels(const std::string &config);
	void process(chart::data::PBars bars);
private:
	FindLevelsParams getLevelsParams(const std::string &section);
	std::vector<Level> findLevels(chart::data::PBars bars, size_t from, size_t to);
